...
11. **Preflight prerequisite checks** — Checks macOS platform, ffmpeg (PATH + Homebrew paths), Full Disk Access (file probe on chat.db), and ElevenLabs API key. Reports ALL failures at once with Rich table and step-by-step fix instructions. Integrated into `main()` and runs automatically before any interactive prompts. Bypassed with `--skip-checks` flag.
