...
- **attributedBody**: Modern macOS stores message text in binary plist format. `parse_attributed_body()` splits on the `NSString` marker, locates the `+` (0x2B) byte, then reads a variable-width integer length using Apple's typedstream encoding: a single byte for lengths 0-127, or 0x81 followed by a 2-byte little-endian uint16 for 128-65535. It then extracts exactly that many bytes and decodes as UTF-8. The length field represents byte count, not character count, which matters for multi-byte UTF-8 characters (e.g., emoji)
